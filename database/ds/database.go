// 1. Quickly operate the database, simple additions, deletions and modifications, no guarantee of reliability, but really fast
// 2. Custom function to handle sql error
// 3. Use the original sql statement
// 4. If you want to use the current package, please set the value of the DB variable first, that is, create a database connection
...
// initialize
func init() {
	// initialize process sql error anonymous function
...
func Query(query string, args ...interface{}) (slices []map[string]interface{}) {
	...
	for rows.Next() {
		...
		for k, v := range vals {
			...
			case "sql.RawBytes":
				tmp[cols[k]] = vs
			default: // default use string type
				tmp[cols[k]] = vs
			}
		}
...
func Get(get interface{}, query string, args ...interface{}) {
	err := errors.New("Parameter must be a struct pointer!")
	...
	for rows.Next() {
		...
		for _, column := range columns {
			column = name.UnderlineToPascal(column)
			field = resultValue.FieldByName(column)
			if field == reflectZeroValue {
				ProcessSqlError(errors.New(fmt.Sprintf("Can not find the corresponding field <%s> found in the struct <%s>!", column, t.Name())))
				return
			}
			if !field.CanSet() {
				ProcessSqlError(errors.New(fmt.Sprintf("Unable to set value, corresponding field <%s> was found in struct <%s>!", column, t.Name())))
				return
			}
...
func GetAll(get interface{}, query string, args ...interface{}) {
	...
	case reflect.Struct:
		...
		for _, column := range columns {
			column = name.UnderlineToPascal(column)
			filed := rowResultValue.FieldByName(column)
			if reflectZeroValue == filed {
				ProcessSqlError(errors.New(fmt.Sprintf("Can not find the corresponding field <%s> found in the struct <%s>!", column, t.Elem().Elem().Name())))
				return
			}
			if !filed.CanSet() {
				ProcessSqlError(errors.New(fmt.Sprintf("Unable to set value, corresponding field <%s> was found in struct <%s>!", column, t.Elem().Elem().Name())))
				return
			}
	...
	case reflect.Ptr:
		...
		for rows.Next() {
			...
			for _, column := range columns {
				...
				filed := rowResultValue.FieldByName(column)
				if reflectZeroValue == filed {
					ProcessSqlError(errors.New(fmt.Sprintf("Can not find the corresponding field <%s> found in the struct <%s>!", column, t.Elem().Elem().Elem().Name())))
					return
				}
				if !filed.CanSet() {
					ProcessSqlError(errors.New(fmt.Sprintf("Unable to set value, corresponding field <%s> was found in struct <%s>!", column, t.Elem().Elem().Elem().Name())))
					return
				}
				scans = append(scans, filed.Addr().Interface())
			}
